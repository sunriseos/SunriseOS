    ...
    fn print_mapping(&mut self) {
        ...
        impl State {
            ...
            fn print(&self, newstate: State) {
                ...
                match *self {
                    State::Present(addr, phys) => writeln!(Loggers, "{:#010x} - {:#010x} - MAPS {:#010x}-{:#010x}", addr, new_vaddr, phys, (phys + (new_vaddr - addr))),
                    State::Guarded(addr) => writeln!(Loggers, "{:#010x} - {:#010x} - GUARDED", addr, new_vaddr),
                    State::Available(addr) => writeln!(Loggers, "{:#010x} - {:#010x} - AVAILABLE", addr, new_vaddr),
                };
            }
        }
